...
def signup():
    ...
    if existing_user:
        res = {'message': 'Could not create account for this email.',
               'errorCode': 1, 'user': None}
        return Response(json.dumps(res), status=400, mimetype='application/json')
...
def login():
    ...
    if not email or not password:
        res = {'message': 'Email or password missing.',
               'errorCode': 1, 'user': None, 'token': None}
    ...
    if bcrypt.check_password_hash(existing_user.get('password'), password):
        ...
        res = {'message': 'User logged in succesfully',
               'errorCode': 0, 'user': user, 'token': token}
...
def get_tasks():
    ...
    if current_user:
        ...
        res = {'message': 'Tasks fetched successfully',
               'errorCode': 0, 'tasks': tasks}

        return Response(json.dumps(res), status=200, mimetype='application/json')

    else:
        res = {'message': 'Authorization failed.',
               'errorCode': 1, 'tasks': None}
...
def create_task():
    ...
    if current_user:
        ...
        res = {'message': 'Task created succesfully',
               'errorCode': 0, 'task': task}
        return Response(json.dumps(res), status=200, mimetype='application/json')

    else:
        res = {'message': 'Authorization failed.',
               'errorCode': 1, 'task': None}
        return Response(json.dumps(res), status=403, mimetype='application/json')
...
def get_task(taskId):
    ...
    if current_user:
        task = db.tasks.find_one(
            {'_id': ObjectId(taskId), 'owner': current_user.get('id')})

        if task:
            task['_id'] = str(task['_id'])

            res = {'message': 'Task fetched successfully',
                   'errorCode': 0, 'task': task}
            return Response(json.dumps(res), status=200, mimetype='application/json')
...
@jwt_required()
def edit_task(taskId: int) -> Response:
    '''
    '''
    current_user = get_jwt_identity()
    ...
    if current_user:
        ...
        result = db.tasks.find_one_and_replace(
            {'_id': ObjectId(taskId), 'owner': current_user.get('id')}, newTask)

        if result:
            res = {'message': 'Task updated successfully.',
                   'errorCode': 0, 'task': newTask}
            return Response(json.dumps(res), status=200, mimetype='application/json')
